...
if (isset($_GET['userid'])) {
    ...
    $stmt = $conn->prepare("UPDATE login SET activation_link='activated' WHERE activation_link=?");
    ...
    require_once("includes/connect_db.php");
    $stmt = $conn->prepare("UPDATE ads SET activation_link='activated' WHERE activation_link=?");
    $stmt->bind_param("s", $link);
    $stmt->execute();
    if ($stmt->affected_rows > 0) {

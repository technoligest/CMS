...
<div class="col-md-4 col-sm-12">
    ...
    for($i=0; $i<$retrieve_category_result->num_rows/3; ++$i){
        ...
        echo "<a class='list-group-item active' href=\"search.php?cat_id={$row['cat_id']}\"><h4>{$row['cat_title']}</h3></a>";
        $sql="SELECT * FROM category_sections WHERE category_sections.cat_id='{$row['cat_id']}'";
        $result = $conn->query($sql);
        while($row = $result->fetch_assoc()){
            echo "<a class=\"list-group-item\" href=\"search.php?cat_section_id={$row['cat_section_id']}\">";

...
function printCategory($row, $conn)
{
    ...
    $return .= "<a class='list-group-item active' href=\"post_ad.php?cat_id={$row['cat_id']}\"><h4>{$row['cat_title']}</h3></a>";
    $sql = "SELECT * FROM categories WHERE cat_parent='{$row['cat_id']}'";
    $result = $conn->query($sql);
    while ($row = $result->fetch_assoc()) {
        $return .= "<a class=\"list-group-item\" href=\"post_ad.php?cat_id={$row['cat_id']}\">{$row['cat_title']}</a>";
    }
    $return .= "</div><br>";
    return $return;
}
